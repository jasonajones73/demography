...
knitr::opts_chunk$set(echo = TRUE, message = FALSE, warning = FALSE, out.width = "80%", dpi = 320, fig.height = 10)
```
...
plot_usmap(data = for_map, values = "change") +
  scale_fill_distiller("Raw Change", type = "div", palette = "Spectral",
                       direction = 1, labels = scales::comma_format()) +
...
```{r}
guil_tracts <- tracts(state = 37, county = 81)

tracts_sf <- guil_tracts %>%
  st_as_sf()
```
...
pop_shares %>%
  ...
  select(1:2, change) %>%
  arrange(desc(change)) %>%
  mutate(change = prettyNum(change, big.mark = ",")) %>%
  DT::datatable() %>%
  DT::formatStyle('change', DT::styleColorBar()) %>%
  DT::saveWidget(file = "C:/Users/jjones6/Desktop/test.html")
```
